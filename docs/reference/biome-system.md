...
Minecraft 1.18은 6개 파라미터를 사용하지만, 본 프로젝트는 현재 **3개 파라미터**로 간소화된 시스템을 구현하고 있습니다.

### 현재 구현: 3-파라미터 시스템

각 파라미터는 독립적인 FBM(Simplex) 노이즈로 생성되며, `[0, 1]` 범위로 샘플링된 후 `[-1, 1]`로 매핑됩니다.

| 파라미터 | 용도 | 월드에 미치는 영향 |
|---------|------|-----------------|
| **Temperature** | 기후 온도 | 더운 바이옴(사막) vs 추운 바이옴(툰드라) |
| **Humidity** | 습도 수준 | 습한 바이옴(숲) vs 건조한 바이옴(사막) |
| **Continentalness** | 육지 vs 바다 | 낮음 = 바다, 중간 = 평원/내륙, 높음 = 산악 |

이 3개를 역할별로 묶으면:

- **기후 축**: Temperature, Humidity — "이 지역의 기후는?"
- **지형 축**: Continentalness — "육지인가, 바다인가? 고도는?"

추가로, `continentalness < oceanThreshold` (Config에서 설정)이면 최근접 이웃 검색 없이 강제로 OCEAN 바이옴이 선택됩니다.

### 향후 확장: 6-파라미터 시스템 (Minecraft 1.18 레퍼런스)

Minecraft의 완전한 Multi-Noise 시스템은 3개 파라미터를 추가로 사용합니다:
...
```
1. 각 블록 위치 (x, z)에 대해 3개 노이즈 파라미터를 샘플링:
     P = (T, H, C)
...
4. 가장 가까운 바이옴이 선택됨
   (단, continentalness < oceanThreshold이면 강제 OCEAN)
```

> **참고**: Minecraft의 6-파라미터 시스템에서는 `P = (T, H, C, E, W, D)` 6차원 거리를 사용합니다. 본 프로젝트는 3-파라미터 부분집합만 구현합니다.
...
### 현재 프로젝트의 바이옴 파라미터 테이블

아래는 `src/terrain/BiomeTypes.ts`의 `BIOME_PARAMS` 실제 값입니다:
...
### Minecraft 레퍼런스: 6-파라미터 테이블 (참고용)

Minecraft 1.18의 완전한 시스템은 Erosion, Weirdness를 포함한 6차원 파라미터를 사용하며, 바이옴당 **다중 포인트 정의**로 비볼록(non-convex)한 영역도 표현합니다.
...
본 프로젝트의 현재 구현에서는 **Continentalness 단독**으로 기본 높이를 결정하고, 별도의 **높이 변동 노이즈**로 국지적 변화를 추가합니다.

Minecraft의 완전한 시스템에서는 3개 파라미터의 조합으로 결정됩니다:
...
#### PV (Peaks & Valleys) 변환 (향후 확장)

> **참고**: PV 변환은 현재 프로젝트에서 미구현입니다. Weirdness 파라미터 도입 시 함께 구현될 예정입니다.
...
바이옴은 현재 3개 파라미터(향후 6개로 확장 가능)를 기반으로 독립적으로 선택됩니다. 이는 동일한 지형 형태가 다른 바이옴을 가질 수 있음을 의미합니다.

예: 같은 Continentalness(내륙 지형)인 지역이 Temperature/Humidity에 따라 사막이 될 수도, 평원이 될 수도 있습니다.
