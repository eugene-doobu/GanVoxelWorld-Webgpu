...
이론적으로 원시 simplex noise 출력은 `[-1, 1]` 범위이지만, 본 프로젝트의 `SimplexNoise` 구현은 **내부적으로 이미 `[0, 1]`로 정규화**하여 반환합니다:

```typescript
// SimplexNoise.ts — noise2D 반환 (2D 스케일 팩터: 70.0)
return (70.0 * (n0 + n1 + n2) + 1.0) * 0.5;

// SimplexNoise.ts — noise3D 반환 (3D 스케일 팩터: 32.0)
return (32.0 * (n0 + n1 + n2 + n3) + 1.0) * 0.5;
```

스케일 팩터 70.0(2D)과 32.0(3D)은 각 차원의 최대 기여도를 경험적으로 정규화하는 값입니다. 호출자는 별도의 정규화 없이 바로 `[0, 1]` 범위의 값을 사용할 수 있습니다.
