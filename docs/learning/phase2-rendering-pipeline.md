...
│       ↓                                                             │
│  5.5 Cloud Raymarch ────────────────> Cloud RGBA16F (half-res)     │
│  5.5 Cloud Temporal ────────────────> Cloud RGBA16F (half-res)     │
│       ↓                                                             │
│  6. Sky Pass ───────────────────────> HDR Texture (cloud composite) │
│       ↓                                                             │
...
| Cloud Raymarch | scene UBO (CloudUniforms), depth | Cloud RGBA16F half-res (scatter + transmittance) |
| Cloud Temporal | cloud current, cloud history, scene UBO | Cloud RGBA16F half-res (reprojected) |
| Sky | depth (sky mask), scene UBO, cloudTexture | HDR (cloud composite + sky pixels) |
| Water | scene color, depth, water UBO | HDR (alpha blend) |
...
struct SceneUniforms {                          // offset (bytes)
  ...
  cloudParams:          vec4<f32>,              // 144 - 159  (reserved, zeroed — 구름은 별도 CloudUniforms 버퍼 사용)
...
구름은 별도 `cloud_raymarch.wgsl` / `cloud_temporal.wgsl` 에서 반해상도로 렌더링되며, sky.wgsl 에서 `@binding(2)` cloudTexture 로 합성된다.
...
### 8.8 볼류메트릭 구름 (SEUS-style Pipeline)

**파일:** `src/renderer/VolumetricClouds.ts`, `src/shaders/cloud_raymarch.wgsl`, `src/shaders/cloud_temporal.wgsl`

구름은 sky.wgsl 에 인라인으로 처리하지 않고 **별도 패스**에서 렌더링된다. 파이프라인:

```
VolumetricClouds (per-frame)
  ├─ cloud_raymarch.wgsl  (반해상도 fragment, 64 steps max)
  │    └─ 결과: Cloud RGBA16F half-res (rgb=scatteredLight, a=transmittance)
  ├─ cloud_temporal.wgsl  (반해상도 fragment, 95% history)
  │    └─ 결과: Cloud RGBA16F half-res (reprojected)
  └─ sky.wgsl에서 합성
       └─ finalColor = skyColor * cloudData.a + cloudData.rgb
```

#### 밀도 함수 (`cloud_raymarch.wgsl:sampleCloudDensity`)

```
1. 높이 그라디언트 (누적란형):
   hGrad = smoothstep(0.0, 0.2, h) * (1.0 - smoothstep(0.5, 1.0, h))

2. 바람 이동:
   windOffset = vec3(windDirX, 0, windDirZ) * windSpeed * time

3. 대형 형태 — 4-octave Simplex FBM:
   fbmShape(pos * 0.003): amplitude = 0.5 → 0.25 → 0.125 → 0.0625

4. Coverage remap (선형):
   remapped = (shape - (1.0 - coverage)) / coverage
   → coverage가 낮을수록 희박, 높을수록 빽빽한 구름

5. Worley 디테일 침식 — 3-octave inverted Worley FBM:
   fbmWorley(pos * 0.012): (1 - worley) 합산 → 누적란형 경계 생성
   eroded = remapped - detail * detailStrength * 0.35

6. 최종 밀도: eroded * hGrad * density * 0.1
```

#### 라이팅 (`lightMarch`, 6 steps)

```
- Beer-Lambert: beer = exp(-opticalDepth)
- Powder sugar effect: powder = 1 - exp(-opticalDepth * 2)
- 최종 라이트 에너지: max(beer * mix(1, powder, 0.5), multiScatterFloor)
- 직접광: lightColor * lightEnergy * (dualHG_phase + silverLining)
- 주변광: ambient * (0.3 + 0.7 * h)  — 구름 상단이 더 밝음
- Energy-conserving integration: (1 - sampleTransmittance) / density
- 조기 종료: transmittance < 0.01
```

#### 위상 함수 (이중 로브 Henyey-Greenstein)

```wgsl
phase = hgPhase(cosTheta, g1) * 0.7 + hgPhase(cosTheta, g2) * 0.3
// g1 = cloudParams3.z (전방 산란), g2 = cloudParams3.w (후방 산란)
```

#### 적응형 스텝

```wgsl
stepCount = mix(64.0, 24.0, smoothstep(0.0, 0.5, abs(rayDir.y)))
// 수평 레이(rayDir.y ≈ 0): 64 steps (구름층 긴 통과)
// 수직 레이(rayDir.y ≈ 1): 24 steps (구름층 빠른 통과)
```

#### Temporal Reprojection (`cloud_temporal.wgsl`)

```
- 95% history / 5% current
- far-plane worldPos → prevViewProj 재투영 (worldDir 기반 아님)
- 3x3 neighborhood AABB clamping (anti-ghosting)
- OOB(out-of-bounds) 시 current만 사용
- Ping-pong 2버퍼: history[0], history[1]
```
...
> **SSAO 아티팩트 정책**: 굴절(아래 보기)에는 `sceneColorTex` 를 절대 사용하지 않는다.
> `sceneColorTex` 에 SSAO 가 bake 된 상태이므로 수중 지오메트리의 SSAO 헤일로가
> 수면에 원형으로 나타난다. 굴절은 절차적 색상만 사용한다.

1. **Procedural refraction (절차적 굴절색):**
```wgsl
...
// waterDepth = linearScene - linearWater (수심)
```
수심에 따라 얕은 청록 → 깊은 남색으로 변화한다.

2. **SSR (Screen-Space Reflections):**
```wgsl
// waterSSR() — 위를 향하는 reflectDir.y > 0.02 인 경우만 처리
// 48-step 선형 마치 + 5-step binary refinement
// 히트 실패 시: analytical sky gradient + sceneColorTex 하늘 픽셀 (depth > 0.999)
```

3. **태양 스펙큘러:**
```wgsl
let specular = frag.sunColor * frag.sunIntensity * pow(sunReflect, 512.0) * 0.3;
// 좁은 태양 글린트만 유지 (넓은 로브 제거)
```

4. **Edge foam (해안선):** `smoothstep(0.5, 0.0, waterDepth)` + 사인파 변조

5. **Fresnel 반사:**
```wgsl
let F0 = F0_DIELECTRIC;  // = 0.02 (비금속 유전체)
let fresnel = F0 + (1.0 - F0) * pow(1.0 - NdotV, 5.0);
let depthBoost = smoothstep(0.0, 3.0, waterDepth) * 0.15;
let finalFresnel = clamp(fresnel + depthBoost, 0.0, 0.40);
// 수심이 깊을수록 반사가 약간 강해지며, 최대 40%로 제한
```
...
| `src/renderer/VolumetricClouds.ts` | 구름 레이마치+템포럴 패스, 바인드 그룹 관리 |
...
| `src/shaders/sky.wgsl` | 하늘, 별, 달, 구름 합성 (cloud texture composite) |
| `src/shaders/cloud_raymarch.wgsl` | 볼류메트릭 구름 레이마치 (Simplex+Worley FBM) |
| `src/shaders/cloud_temporal.wgsl` | 구름 템포럴 리프로젝션 (95% history) |
